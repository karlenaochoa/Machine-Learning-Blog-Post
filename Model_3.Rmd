...
1. Tuned the learning rate between the optimum values of .005 to .3.
...
First, we split the training dataset into training and testing sets and set-up cross validation. Not shown below is that we used only 5% of the training dataset, as it took too long for our model to run otherwise. This can be done by adding "sample_frac(.05)" when reading the data in (even with 5%, all these models took 7.5 hours to run!).
...
Second, we write our recipe. This is the same as previous recipes used in prior models. 
...
Third, we set-up our model. For boosted trees, we are using the "xgboost" engine. Also, "set_mode" is how we tell our model that we are running a regression. If we were doing a classification model, we would tell the model that here.
...
We then add our model to a workflow. This will be helpful as we change our model by tuning different hyperparemters throughout the model fitting process, as we can simply update this workflow with that information as opposed to starting from scratch with new recipes, models, etc.
...
We started tuning at .005 to .3 because smaller learning rates are seen as better and this range is a default starting point. You can see that we established tuning by telling our model that "learn_rate" is set to "tune()" and then providing the tuning range through "expand.grid."
...
Then we looked at the output and found the best value from the model. We assessed "best" by looking for the lowest RSME score. You can see the lowest RSME this model produced is X at X learning rate.
...
We decided to plot this output to see if there is any more tuning we can do of the learning rate parameter. After looking at the plot, we decided to try an even smaller learning rate, as our best value was at the bottom of the range we used.
...
The results below demonstrate that this did not result in a better RSME score. Therefore, we decided to build the model using the learning rate that lead to the lowest RSME score in the previous fit and tuning grid.
...
We then decided to tune loss reduction by again setting tha tparameter to tune within the the set_args() function of the model set-up and set-up the range in expand.grid to be from 0 to 100 with intervals of 5. We took the best parameters from the last model as well, and manually set tree_depth to 1.
...
The below code showed that the best model had an RSME of X at an mtry of X and a sample_size of X.
...
We received a score of X, better than our preliminary fit 2!
