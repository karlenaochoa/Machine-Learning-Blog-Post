...
As seen in the plot, latitude and longitude do not seem to be a variable with a lot of importance in our model, and instead grade in school and whether a child participated in a special education program, or talented and gifted program seemed to be the most important variables of the ones examined. The salmon color bars represent negative values, and teal bars represent positive values. 

This first model allowed us to get acquainted with ML and specifically using recipes. We starting to use some feature engineering to make a more predictive model and will continue to do so in subsequent models. 
